...
if (!isMainInstance) {
  ...
  */
  const checkForUpdatesA6 = () => {
    currentVersionA6 = JSON.parse(fs.readFileSync(`${dataDir}/pokeclicker-acsrq-beta/docs/a6csrq.json`).toString()).version;
    const request = https.get('https://raw.githubusercontent.com/switchlove/pokeclicker/acsrq-beta/docs/a6csrq.json', res => {
      let body = '';
  ...
  const downloadUpdateA6 = async => {
    const file = fs.createWriteStream(`${dataDir}/pokeclicker-acsrq-beta/docs/scripts/a6csrq.js`);

    https.get('https://raw.githubusercontent.com/switchlove/pokeclicker/acsrq-beta/docs/scripts/a6csrq.js', async res => {
      res.pipe(file).on('finish', async () => {
        try {
          if (initial) await mainWindow.webContents.executeJavaScript('setStatus("Files Downloaded!<br/>Extracting Files...")');
        ...
        const fileName1 = `${dataDir}/pokeclicker-acsrq-beta/docs/a6csrq.json`;
