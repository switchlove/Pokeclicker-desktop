...
if (!isMainInstance) {
  ...
  */
  ...
  const downloadUpdateA6 = async => {
    const file = fs.createWriteStream(`${dataDir}/pokeclicker-acsrq-beta/docs/scripts/acsrq.js`);

    https.get('https://raw.githubusercontent.com/switchlove/pokeclicker/acsrq-beta/docs/scripts/acsrq.js', async res => {
      res.pipe(file).on('finish', async () => {
        try {
          if (initial) await mainWindow.webContents.executeJavaScript('setStatus("Files Downloaded!<br/>Extracting Files...")');
